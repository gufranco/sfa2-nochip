...
class PatchShapeTest(unittest.TestCase):
    ...
    def test_the_runs_are_ordered_and_disjoint(self):
        end = -1
        for at, data in spcfast.PATCH:
            self.assertGreater(at, end)
            end = at + len(data) - 1

    def test_no_run_touches_the_checksum_field(self):
        for at, data in spcfast.PATCH:
            self.assertFalse(at <= spcfast.CHECKSUM_FIELD < at + len(data))

    def test_every_run_stays_inside_a_four_megabyte_rom(self):
        for at, data in spcfast.PATCH:
            self.assertLessEqual(at + len(data), 0x400000)

    def test_the_patch_reaches_the_sound_bank(self):
        self.assertIn(0x07, {at >> 16 for at, _ in spcfast.PATCH})

    def test_the_blank_gate_is_the_documented_sequence(self):
...
class DriverTest(unittest.TestCase):
    ...
    def test_the_receive_loop_is_inside_the_patch(self):
...
class ChecksumTest(unittest.TestCase):
    def test_the_stamped_field_is_a_complement_pair(self):
...
class ApplyTest(unittest.TestCase):
    ...
    def test_a_rom_that_is_not_stock_is_rejected(self):
...
class RetailRomTest(unittest.TestCase):
    ...
    def test_every_patched_region_is_identical_across_regions(self):
        for at, data in spcfast.PATCH:
            self.assertEqual(
    ...
    def test_both_regions_have_seventeen_blank_gates(self):
        self.assertEqual(len(spcfast.find_blank_gates(self.usa)), 17)
        self.assertEqual(len(spcfast.find_blank_gates(self.jp)), 17)

    def test_patching_touches_only_the_patch_and_the_checksum(self):
    ...
    def test_the_patched_block_start_waits_for_a_zero_counter(self):
        patched = spcfast.apply(self.usa)
