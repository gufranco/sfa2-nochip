...
def load_module(name, where=ROOT):
...
class ScanPatternTest(unittest.TestCase):
    ...
    def test_a_scan_line_is_recognised(self):
        self.assertIsNotNone(converge.SCAN.match(self.LINE))

    def test_the_requested_address_is_the_first_field(self):
        self.assertEqual(converge.SCAN.match(self.LINE).group(1), "104C")

    def test_a_line_that_is_not_a_scan_is_ignored(self):
        self.assertIsNone(converge.SCAN.match("SCANLEN addr=104C steps=12"))


class WindowTest(unittest.TestCase):
    def test_the_window_starts_at_bank_c0(self):
        self.assertEqual(converge.WINDOW_BASE, 0xC0)

    def test_the_scan_budget_matches_the_gate(self):
