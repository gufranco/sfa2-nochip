...
class ReadTest(unittest.TestCase):
    ...
    def test_the_totals_come_from_the_line_that_states_them(self):
        with tempfile.TemporaryDirectory() as folder:
            found = compare_audio.read(_log(folder, CLEAN))

            self.assertEqual((found["ok"], found["bad"]), (2, 0))

    def test_a_corrupt_block_is_recorded_with_where_it_went_wrong(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_how_far_the_run_got_is_recorded(self):
        with tempfile.TemporaryDirectory() as folder:
            found = compare_audio.read(_log(folder, CLEAN))

            self.assertEqual((found["load"], found["frames"]), ("ok", 3000))

    def test_a_log_with_nothing_in_it_says_it_does_not_know(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_bytes_it_cannot_decode_do_not_stop_the_read(self):
        with tempfile.TemporaryDirectory() as folder:
...
class UploadTest(unittest.TestCase):
    ...
    def test_a_source_uploaded_twice_is_counted_twice(self):
        with tempfile.TemporaryDirectory() as folder:
            found = self._read(folder, CLEAN + CLEAN, "twice.txt")

            self.assertEqual(compare_audio.uploads(found)[(0xC7204E, 1000)], 2)

    def test_a_build_compared_with_itself_skipped_nothing(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_source_uploaded_less_often_is_reported_with_both_counts(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_source_never_uploaded_at_all_is_reported_separately(self):
        with tempfile.TemporaryDirectory() as folder:
            stock = self._read(folder, CLEAN, "a.txt")
...
class CostTest(unittest.TestCase):
    def test_a_run_that_uploaded_nothing_costs_nothing(self):
        self.assertEqual(compare_audio.cost({"blocks": []}), 0.0)

    def test_the_cost_is_writes_over_bytes(self):
        with tempfile.TemporaryDirectory() as folder:
            found = compare_audio.read(_log(folder, CLEAN))

            self.assertAlmostEqual(compare_audio.cost(found), 2600 / 1300)

    def test_a_faster_loop_costs_less_per_byte(self):
        with tempfile.TemporaryDirectory() as folder:
...
class VerdictTest(unittest.TestCase):
    ...
    def test_a_faster_build_that_delivered_everything_passes(self):
        with tempfile.TemporaryDirectory() as folder:
            stock = self._read(folder, CLEAN, "a.txt")
    ...
    def test_a_corrupt_block_fails(self):
        with tempfile.TemporaryDirectory() as folder:
            stock = self._read(folder, CLEAN, "a.txt")
    ...
    def test_a_source_that_stopped_being_uploaded_fails(self):
        with tempfile.TemporaryDirectory() as folder:
            stock = self._read(folder, CLEAN, "a.txt")
            patched = self._read(folder, SKIPPED, "b.txt")
    ...
    def test_a_build_that_did_not_load_fails(self):
        with tempfile.TemporaryDirectory() as folder:
            stock = self._read(folder, CLEAN, "a.txt")
...
class MainTest(unittest.TestCase):
    ...
    def test_it_refuses_a_roster_that_is_not_a_number(self):
        self.assertEqual(
            compare_audio.main(["compare_audio.py", "--roster", "many", "a.sfc", "b.sfc"]), 2
        )

    def test_it_refuses_a_flag_with_nothing_after_it(self):
