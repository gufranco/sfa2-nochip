...
class DirectoryTest(unittest.TestCase):
    def test_an_entry_reads_back_the_addresses_it_was_given(self):
        ram = blank()
    ...
    def test_the_directory_wraps_inside_the_sixty_four_kilobytes(self):
        ram = blank()
...
class ChainTest(unittest.TestCase):
    def test_a_chain_ends_where_its_last_block_says_it_does(self):
        ram = blank()
    ...
    def test_a_chain_covers_every_byte_of_every_block_it_walked(self):
        ram = blank()
    ...
    def test_a_chain_that_never_ends_is_reported_rather_than_walked_forever(self):
        ram = blank()
    ...
    def test_a_chain_starting_past_the_end_is_refused(self):
        found = audit.chain(blank(), audit.APU_RAM - 4)

        self.assertEqual(found.fault, audit.RUNS_OFF)

    def test_a_chain_filling_the_whole_memory_stops_at_the_last_block(self):
        found = audit.chain(blank(), 0x0000)

        self.assertEqual(found.fault, audit.RUNS_OFF)
        self.assertEqual(found.blocks, audit.MAX_BLOCKS)

    def test_a_chain_of_one_block_is_a_chain(self):
        ram = blank()
...
class PlayabilityTest(unittest.TestCase):
    def test_a_well_formed_sample_produces_sound_through_the_chip(self):
        ram = blank()
    ...
    def test_a_sample_of_silence_produces_none(self):
        ram = blank()
    ...
    def test_the_chip_reports_reaching_the_end_of_a_short_sample(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=1)

        self.assertTrue(audit.reaches_end(ram, DIRECTORY, 0))

    def test_and_does_not_for_one_that_loops_forever(self):
        ram = blank()
...
class AuditTest(unittest.TestCase):
    def test_a_bank_whose_samples_are_all_well_formed_reports_nothing(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_a_sample_that_runs_off_the_end_is_named(self):
        ram = blank()
    ...
    def test_a_loop_point_outside_the_sample_is_named(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_a_loop_point_off_the_block_grid_is_named(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_a_sample_nobody_uses_is_not_audited(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
...
class OverlapTest(unittest.TestCase):
    def test_two_samples_that_do_not_touch_report_nothing(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
        put_sample(ram, 1, FIRST + 0x100, blocks=2)
    ...
    def test_two_samples_sharing_bytes_are_reported_as_a_pair(self):
        ram = blank()
    ...
    def test_a_sample_deliberately_shared_by_two_entries_is_not_an_overlap(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
...
class WriteCollisionTest(unittest.TestCase):
    def test_an_upload_clear_of_every_playing_sample_is_safe(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_an_upload_landing_on_a_playing_sample_is_reported(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_an_upload_touching_the_directory_entry_itself_is_reported(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_a_sample_that_is_not_playing_may_be_overwritten(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
        put_sample(ram, 1, FIRST + 0x100, blocks=2)
...
class ReportTest(unittest.TestCase):
    def test_a_clean_bank_says_how_many_it_looked_at(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
    ...
    def test_a_bank_with_a_fault_names_it(self):
        ram = blank()
    ...
    def test_a_bank_with_an_overlap_names_the_pair(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=4)
        put_directory(ram, 1, FIRST + audit.BLOCK_BYTES, FIRST + audit.BLOCK_BYTES)
...
class EntryPointTest(unittest.TestCase):
    ...
    def test_an_image_and_a_directory_are_enough(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)

        self.assertEqual(audit.main([str(self.image(ram)), hex(DIRECTORY), "0"]), 0)

    def test_naming_no_samples_looks_at_every_entry(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)
...
class ReadingTest(unittest.TestCase):
    def test_a_chain_prints_as_where_it_started_and_how_far_it_got(self):
        ram = blank()
        put_sample(ram, 0, FIRST, blocks=2)

        self.assertIn("2 blocks", repr(audit.chain(ram, FIRST)))

    def test_a_chain_that_did_not_end_says_so(self):
        self.assertIn(audit.RUNS_OFF, repr(audit.chain(blank(), audit.APU_RAM - 4)))

    def test_two_faults_naming_the_same_thing_are_the_same_fault(self):
        first = audit.Fault(sample=3, fault=audit.RUNS_OFF, at=0x1500)
        second = audit.Fault(sample=3, fault=audit.RUNS_OFF, at=0x1500)

        self.assertEqual([first], [second])

    def test_and_two_naming_different_samples_are_not(self):
        first = audit.Fault(sample=3, fault=audit.RUNS_OFF, at=0x1500)
        second = audit.Fault(sample=4, fault=audit.RUNS_OFF, at=0x1500)

        self.assertNotEqual([first], [second])

    def test_a_fault_prints_as_the_sample_and_what_is_wrong_with_it(self):
...
class LoadTest(unittest.TestCase):
    def test_an_apu_image_of_the_wrong_size_is_refused(self):
        with self.assertRaises(audit.Unreadable):
            audit.load(bytes(16))

    def test_an_image_of_the_right_size_is_taken_as_it_is(self):
