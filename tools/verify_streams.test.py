...
class BatchTest(unittest.TestCase):
    def test_nothing_to_check_is_no_batches(self):
        self.assertEqual(verify_streams.batches([], size=10), [])

    def test_fewer_cases_than_a_batch_is_one_batch(self):
    ...
    def test_a_remainder_becomes_a_shorter_last_batch(self):
        self.assertEqual(verify_streams.batches([1, 2, 3], size=2), [[1, 2], [3]])

    def test_every_case_appears_exactly_once(self):
    ...
    def test_no_batch_is_longer_than_the_size_asked_for(self):
        for batch in verify_streams.batches(list(range(1000)), size=7):
            self.assertLessEqual(len(batch), 7)

    def test_the_default_size_is_the_one_the_module_names(self):
