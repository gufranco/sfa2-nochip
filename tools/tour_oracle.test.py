...
class RequestTest(unittest.TestCase):
    def test_a_fixed_source_inside_the_window_is_a_request(self):
        found = tour_oracle.requests([_dma(BASE + 0x19, 0x104C, 832)])

        self.assertEqual(found, {0x19104C: 832})

    def test_a_source_that_advances_is_not_a_decompression(self):
        self.assertEqual(tour_oracle.requests([_dma(BASE + 0x19, 0x104C, 832, fixed=0)]), {})

    def test_a_source_below_the_window_is_not_cartridge_data(self):
        self.assertEqual(tour_oracle.requests([_dma(0x7E, 0x104C, 832)]), {})

    def test_the_largest_length_asked_for_from_one_source_wins(self):
    ...
    def test_a_later_smaller_request_does_not_shrink_the_answer(self):
        found = tour_oracle.requests(
    ...
    def test_two_sources_are_two_requests(self):
        found = tour_oracle.requests(
    ...
    def test_any_channel_can_carry_a_request(self):
        found = tour_oracle.requests([_dma(BASE + 0x19, 0x104C, 832, channel=7)])

        self.assertEqual(found, {0x19104C: 832})

    def test_the_first_bank_of_the_window_is_the_first_bank_of_the_file(self):
        found = tour_oracle.requests([_dma(BASE, 0x8000, 64)])

        self.assertEqual(found, {0x008000: 64})

    def test_lines_that_are_not_transfers_are_ignored(self):
        found = tour_oracle.requests(["RESULT load=ok frames=1", "", _dma(BASE, 0x8000, 64)])

        self.assertEqual(found, {0x008000: 64})

    def test_an_empty_log_asks_for_nothing(self):
        self.assertEqual(tour_oracle.requests([]), {})


class PassTest(unittest.TestCase):
    def test_every_pass_names_itself_once(self):
        names = [name for name, _, _ in tour_oracle.PASSES]

        self.assertEqual(len(names), len(set(names)))

    def test_the_frame_count_of_each_pass_is_the_budget_times_the_roster(self):
    ...
    def test_at_least_one_pass_enters_fights(self):
        self.assertTrue(any("SFTOURCONFIRM=1" in extra for _, extra, _ in tour_oracle.PASSES))

    def test_the_passes_do_not_all_use_the_same_budget(self):
        budgets = {
...
class OracleTest(unittest.TestCase):
    def test_the_oracle_is_a_cartridge_form_image(self):
        self.assertTrue(tour_oracle.ORACLE.endswith("-cart.sfc"))

    def test_the_oracle_carries_no_skip_patch(self):
    ...
    def test_the_window_base_is_the_one_every_tool_here_uses(self):
