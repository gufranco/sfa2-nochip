...
class DifferingRunsTest(unittest.TestCase):
    def test_identical_images_produce_no_runs(self):
        image = bytes(64)

        self.assertEqual(freeze.differing_runs(image, image), [])

    def test_one_changed_byte_is_one_run(self):
        before = bytes(64)
    ...
    def test_adjacent_changes_join_into_one_run(self):
        before = bytes(64)
        after = bytearray(before)
    ...
    def test_a_gap_splits_the_runs(self):
        before = bytes(64)
        after = bytearray(before)
        after[10] = 0x01
        after[12] = 0x02
    ...
    def test_the_checksum_field_is_never_recorded(self):
        before = bytes(0x8000)
        after = bytearray(before)
    ...
    def test_a_run_touching_the_checksum_field_stops_at_it(self):
        before = bytes(0x8000)
        after = bytearray(before)
        after[0x007FDA] = 0x01
        after[0x007FDB] = 0x02
...
class RenderTest(unittest.TestCase):
    ...
    def test_the_rendered_table_is_valid_python_and_round_trips(self):
