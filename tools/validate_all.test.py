...
class BurstTest(unittest.TestCase):
    def test_no_frames_is_no_burst(self):
        self.assertEqual(validate_all.longest_burst([]), 0)

    def test_one_frame_is_a_burst_of_one(self):
        self.assertEqual(validate_all.longest_burst([10]), 1)

    def test_consecutive_frames_are_one_burst(self):
        self.assertEqual(validate_all.longest_burst([10, 11, 12]), 3)

    def test_a_gap_ends_a_burst(self):
        self.assertEqual(validate_all.longest_burst([10, 11, 20, 21, 22]), 3)

    def test_the_longest_burst_wins_even_when_it_comes_first(self):
        self.assertEqual(validate_all.longest_burst([1, 2, 3, 4, 10]), 4)

    def test_a_repeated_frame_number_does_not_extend_a_burst(self):
...
class SummariseTest(unittest.TestCase):
    ...
    def test_a_clean_run_reports_what_it_loaded_and_how_far_it_got(self):
    ...
    def test_a_bright_sample_above_the_threshold_counts_as_lit(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_scan_within_the_budget_is_not_a_miss(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_scan_past_the_budget_is_a_miss(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_run_of_audio_frames_becomes_a_pause_in_seconds(self):
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_a_log_with_no_audio_frames_reports_no_pause(self):
        with tempfile.TemporaryDirectory() as folder:
            found = validate_all.summarise(self._log(folder, "RESULT load=ok frames=1\n"))

            self.assertEqual(found["pause"], "0.00s")

    def test_bytes_it_cannot_decode_do_not_stop_the_summary(self):
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / "run.txt"
...
class MappingTest(unittest.TestCase):
    ...
    def test_the_two_forms_are_read_through_different_maps(self):
        self.assertNotEqual(
            validate_all.mapping_for("jp-both-free"), validate_all.mapping_for("jp-both-cart")
        )

    def test_the_comparison_tool_reads_cartridge_form_the_same_way(self):
