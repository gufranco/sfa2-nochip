...
class MatrixTest(unittest.TestCase):
    def test_every_region_names_a_retail_cartridge_and_a_bypass(self):
        self.assertEqual(sorted(rebuild_all.RETAIL), sorted(rebuild_all.BYPASS))

    def test_the_prefight_table_goes_into_exactly_one_variant(self):
...
class VariantTest(unittest.TestCase):
    ...
    def test_four_variants_are_produced(self):
        self.assertEqual(sorted(self.variants), ["base", "both", "sa", "spc"])

    def test_the_base_variant_is_the_retail_cartridge_untouched(self):
        self.assertEqual(self.variants["base"], self.retail)

    def test_every_variant_is_the_same_size_as_the_cartridge(self):
        for name, image in self.variants.items():
            self.assertEqual(len(image), len(self.retail), name)

    def test_every_variant_other_than_the_base_differs_from_it(self):
        for name, image in self.variants.items():
            if name == "base":
                continue
            self.assertNotEqual(image, self.retail, name)

    def test_no_two_variants_are_the_same_image(self):
        seen = {}
        for name, image in self.variants.items():
            self.assertNotIn(image, seen.values(), name)
            seen[name] = image

    def test_the_fast_upload_variant_carries_the_fast_upload_patch(self):
    ...
    def test_the_everything_variant_carries_the_skip_patch_as_well(self):
    ...
    def test_the_everything_variant_carries_the_unlock(self):
...
class EntryTest(unittest.TestCase):
    def test_each_region_resolves_to_a_non_empty_set_of_streams(self):
        for region in rebuild_all.RETAIL:
            self.assertGreater(len(rebuild_all.entries_for(region)), 0, region)

    def test_the_two_regions_resolve_to_different_sets(self):
