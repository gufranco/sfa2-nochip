...
class MaskTest(unittest.TestCase):
    def test_stream_bytes_are_marked_as_data(self):
        rom = bytes(4096)
    ...
    def test_an_entry_of_unknown_length_is_skipped(self):
        rom = bytes(4096)
...
class RegisterScanTest(unittest.TestCase):
    def test_a_write_to_the_arm_register_is_found(self):
    ...
    def test_a_write_buried_in_compressed_data_is_ignored(self):
        rom = b"\x00" * 16 + b"\x8d\x01\x48" + b"\x00" * 16
        mask = bytearray(len(rom))
        mask[16] = 1
    ...
    def test_the_whole_register_block_is_scanned(self):
...
class RealRomTest(unittest.TestCase):
    ...
    def test_the_arm_register_is_written_from_exactly_seven_places(self):
    ...
    def test_no_register_write_hides_inside_compressed_data(self):
        for found in self.found:
            self.assertFalse(self.mask[found.offset])

    def test_every_arm_site_is_followed_by_a_dma_start(self):
    ...
    def test_the_window_resynchronises_onto_the_site(self):
        listing = sites.window(self.rom, KNOWN_ARM_OFFSETS[0])

        self.assertIn(KNOWN_ARM_OFFSETS[0], [i.offset for i in listing])

    def test_every_arm_site_sits_in_a_bank_reachable_as_lorom_code(self):
        for offset in KNOWN_ARM_OFFSETS:
