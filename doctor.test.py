...
class DecompressorTest(unittest.TestCase):
    ...
    def test_the_report_says_whether_the_decompressor_runs(self):
        self.assertIn("decompressor", [one.name for one in doctor.examine()])

    def test_one_that_decodes_is_reported_with_how_much_came_out(self):
        found = doctor._decompressor(lambda: b"\x00" * 8)

        self.assertTrue(found.ok)
        self.assertIn("8 bytes", found.detail)

    def test_one_that_throws_is_reported_rather_than_swallowed(self):
        def boom():
...
class ToolTest(unittest.TestCase):
    def test_a_tool_that_is_here_is_reported_with_where_it_is(self):
    ...
    def test_one_that_is_not_says_so_and_says_what_still_works(self):
    ...
    def test_every_tool_a_build_shells_out_to_is_reported(self):
        names = [one.name for one in doctor.examine()]
