...
class TableTest(unittest.TestCase):
    def test_every_fix_replaces_a_run_of_its_own_length(self):
        for fix in gamefixes.FIXES:
            self.assertEqual(len(fix.stock), len(fix.patched), fix.name)

    def test_every_fix_actually_changes_something(self):
        for fix in gamefixes.FIXES:
            self.assertNotEqual(fix.stock, fix.patched, fix.name)

    def test_every_fix_names_the_regions_it_reaches(self):
        for fix in gamefixes.FIXES:
...
class LocationTest(unittest.TestCase):
    def test_a_run_present_once_is_found(self):
        fix = gamefixes.FIXES[0]

        self.assertEqual(gamefixes.locate(carrier(fix), fix.stock), 64)

    def test_a_run_that_is_absent_reports_nothing(self):
        self.assertIsNone(gamefixes.locate(b"\x00" * 64, b"\xaa\xbb"))

    def test_a_run_present_twice_is_refused(self):
        fix = gamefixes.FIXES[0]
...
class ApplyTest(unittest.TestCase):
    def test_a_stock_run_becomes_the_patched_run(self):
        fix = gamefixes.FIXES[0]
    ...
    def test_a_rom_carrying_no_fix_at_all_is_refused(self):
        with self.assertRaises(ValueError):
            gamefixes.apply(b"\x00" * 4096)

    def test_the_report_names_every_fix_and_its_state(self):
        fix = gamefixes.FIXES[0]
...
class EmptyCallTest(unittest.TestCase):
    ...
    def test_every_call_followed_by_a_return_is_found(self):
    ...
    def test_a_call_not_followed_by_a_return_is_left_alone(self):
        call = gamefixes.EMPTY_CALLS[0]
    ...
    def test_a_target_that_is_not_a_bare_return_is_refused(self):
        call = gamefixes.EMPTY_CALLS[0]
        rom = bytearray(self.rom_with(call, (0x1000,)))
    ...
    def test_applying_turns_the_call_opcode_into_a_return(self):
        call = gamefixes.EMPTY_CALLS[0]
    ...
    def test_a_retired_site_is_reported_as_already_done(self):
        call = gamefixes.EMPTY_CALLS[0]
        rom = self.rom_with(call, (0x1000,))
    ...
    def test_a_long_address_maps_to_its_window_offset(self):
...
class RetailTest(unittest.TestCase):
    def test_every_empty_call_is_retired_in_both_regions(self):
    ...
    def test_every_empty_call_target_is_a_bare_return(self):
        rom = retail(USA)
    ...
    def test_the_three_routines_together_reach_forty_five_sites(self):
        rom = retail(USA)
    ...
    def test_every_fix_that_claims_usa_is_present_in_the_usa_rom(self):
        rom = retail(USA)
    ...
    def test_every_fix_that_claims_jp_is_present_in_the_jp_rom(self):
        rom = retail(JP)
    ...
    def test_the_usa_rom_changes_by_exactly_the_declared_bytes(self):
        rom = retail(USA)
    ...
    def test_the_jp_rom_changes_by_exactly_the_declared_bytes(self):
        rom = retail(JP)
    ...
    def test_the_usa_rom_changes_more_bytes_than_the_jp_one(self):
    ...
    def test_the_patched_rom_carries_a_consistent_checksum(self):
        rom = retail(USA)
    ...
    def test_applying_to_an_already_patched_usa_rom_is_a_no_op(self):
        rom = retail(USA)
    ...
    def test_every_sodom_plate_is_confined_to_one_region(self):
    ...
    def test_the_rename_reaches_every_screen_that_shows_the_name(self):
    ...
    def test_a_fix_names_a_run_that_appears_once_in_every_region_it_claims(self):
    ...
    def test_a_region_a_fix_does_not_claim_never_carries_the_stock_run(self):
        for path, region in ((USA, "usa"), (JP, "jp")):
            rom = retail(path)
    ...
    def test_the_japanese_rom_already_reads_right_where_it_shares_the_run(self):
        rom = retail(JP)
    ...
    def test_the_vs_results_fix_only_turns_katana_into_sodom(self):
    ...
    def test_no_fix_writes_over_the_start_of_a_neighbouring_stream(self):
    ...
    def test_the_vs_results_fix_stops_where_the_next_stream_begins(self):
        usastreams = load_module("usastreams")
        rom = retail(USA)
        fix = next(f for f in gamefixes.FIXES if f.name == "sodom name on the vs results screen")
        at = gamefixes.locate(rom, fix.stock)
    ...
    def test_the_vs_results_fix_zero_fills_the_bytes_the_shorter_stream_leaves(self):
        fix = next(f for f in gamefixes.FIXES if f.name == "sodom name on the vs results screen")

        self.assertEqual(len(fix.patched), len(fix.stock))
        self.assertEqual(fix.patched[-6:], b"\x00" * 6)

    def test_the_akuma_pose_fixes_reach_both_regions(self):
    ...
    def test_the_reorder_puts_the_taunt_first_and_drops_the_dead_entry(self):
    ...
    def test_the_silent_pose_index_matches_where_the_reorder_puts_it(self):
    ...
    def test_the_object_table_fix_reaches_both_regions(self):
