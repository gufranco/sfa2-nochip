...
class BitmapTest(unittest.TestCase):
    def test_every_window_of_the_source_is_marked(self):
        data = bytes(range(64))
    ...
    def test_an_absent_window_is_usually_rejected(self):
        data = bytes(range(64))
    ...
    def test_a_bitmap_sized_in_bits_allocates_that_many_slots(self):
...
class EntropyTest(unittest.TestCase):
    def test_flat_output_is_rejected(self):
        self.assertFalse(find.is_distinctive(b"\x00" * 64, minimum=10))

    def test_a_two_value_pattern_is_rejected(self):
        self.assertFalse(find.is_distinctive(b"\x01\x02" * 32, minimum=10))

    def test_real_tile_data_is_accepted(self):
        rng = random.Random(9)
...
class FindStreamsTest(unittest.TestCase):
    ...
    def test_a_known_stream_is_located_in_a_synthetic_reference(self):
    ...
    def test_the_hit_reports_where_the_output_landed(self):
        reference = self.build_reference()
    ...
    def test_a_reference_without_the_stream_yields_nothing(self):
        rng = random.Random(2)
    ...
    def test_a_stream_that_opens_on_a_blank_tile_is_not_locatable(self):
